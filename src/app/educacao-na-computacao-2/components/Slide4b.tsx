export default function Slide4b() {
  return (
    <section className="flex flex-col gap-4">
      <h2>Contagem de Cartas</h2>

      <div className="flex flex-row gap-8 justify-center">
        <article className="flex flex-col">
        <a
          href="https://drive.google.com/file/d/1aDa8SPbw9WhAtJ2q0a32POQXuAIDilwn/view?usp=sharing"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            src="/img-01-cover.png"
            alt="A Favorable Strategy for Twenty-One by Edward Thorp"
            width="400"
            height="520"
            loading="lazy"
          />
        </a>
        <p><strong>A Favorable Strategy for Twenty-One</strong></p>
        <dl>
          <dt>Autor:</dt>
          <dd>Edward Thorp (1961)</dd>
          <dt>Revista:</dt>
          <dd>PNAS</dd>
          <dt>Referência:</dt>
          <dd>Vol. 47, No. 1, pp. 110-112</dd>
          <dt>DOI:</dt>
          <dd>10.1073/pnas.47.1.110</dd>
          <dt>Link:</dt>
          <dd>
            <a
              href="https://drive.google.com/file/d/1aDa8SPbw9WhAtJ2q0a32POQXuAIDilwn/view?usp=sharing"
              target="_blank"
              rel="noopener noreferrer"
            >
              drive.google.com/file/d/1aDa8SPbw9WhAtJ2q0a32POQXuAIDilwn
            </a>
          </dd>
        </dl>
        </article>

        <article className="flex flex-col">
        <a
          href="https://drive.google.com/file/d/1qDfbnwQFV2zLpMAqyY0NilPej4OewYa4/view?usp=sharing"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            src="/img-00-cover.png"
            alt="Mathematics of Big Four"
            width="400"
            height="520"
            loading="lazy"
          />
        </a>
        <p><strong>Mathematics of The Big Four Casino Table Games</strong></p>
        <dl>
          <dt>Autor:</dt>
          <dd>Mark Bollman (2021)</dd>
          <dt>Revista:</dt>
          <dd>Technometrics</dd>
          <dt>Referência:</dt>
          <dd>Vol. 65, No. 4, 2023</dd>
          <dt>Publisher:</dt>
          <dd>Taylor &amp; Francis</dd>
          <dt>Link:</dt>
          <dd>
            <a
              href="https://drive.google.com/file/d/1qDfbnwQFV2zLpMAqyY0NilPej4OewYa4/view?usp=sharing"
              target="_blank"
              rel="noopener noreferrer"
            >
              drive.google.com/file/d/1qDfbnwQFV2zLpMAqyY0NilPej4OewYa4
            </a>
          </dd>
        </dl>
        </article>
      </div>
    </section>
  );
}
