...
/**
 * API para buscar estatísticas dos jogadores de Blackjack Hi-Lo
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query Firestore
    const playersRef = collection(db, 'blackjack-hi-lo-players');
    ...
    const processedPlayers: ProcessedPlayer[] = allPlayers.map((player: PlayerData) => {
      ...
      const balanceBefore = player.balanceBefore ?? 100;
