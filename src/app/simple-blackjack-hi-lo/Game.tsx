...
interface GameProps {
  player: PlayerData;
  onGameEnd: (gamesPlayed: number) => void;
...
export default function Game({ player, onGameEnd, initialGameState, initialStats }: GameProps) {
  ...
  // Partner's count with +/- 2 error
  const getPartnerCount = () => {
    const error = Math.floor(Math.random() * 5) - 2; // Random from -2 to +2
    return gameState.hiLoCount + error;
  };
  ...
  const handleGameEnd = async () => {
    ...
    onGameEnd(gameState.gamesPlayed);
  };
          ...
          <div className="flex gap-4">
            ...
            <div className="text-center">
              <motion.button
                ...
                className="relative w-16 h-24 bg-purple-50 rounded border-2 border-purple-400 flex flex-col items-center justify-center mb-1 cursor-pointer hover:bg-purple-100 transition-colors"
              ...
              >
                ...
                {showHint && (
                  ...
                  >
                    {getPartnerCount() > 0 ? '+' : ''}{getPartnerCount()}
                  </motion.span>
                )}
              </motion.button>
              <p className="text-xs text-gray-600 font-light">Parceiro</p>
            </div>
          </div>
        </div>
